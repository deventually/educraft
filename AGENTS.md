...
- **Model availability uses INTERSECT, three levels (P13).** `getSelectableModelIds(user)`/`getSelectableModels(user)` narrow the instance base (client-selectable catalog ∩ admin `enabledModels`) by the caller's role: a **teacher** ∩ their `assignedModels:<userId>` (admin write UI: per-teacher section of `admin.models.tsx`), a **student** ∩ their cohort's `allowedModelsJson` (teacher write UI: `cohorts.$id.tsx`). Empty/unset at a level = inherit the level above; one lockout fallback to `DEFAULT_MODEL` after. Enforced in the stream (`getSelectableModelIds(user)`) + offered in the tool loader (`getSelectableModels(viewer)`). **Unlike the P12 context axes (override), models only ever narrow** — a billing/safety cap, so a teacher/cohort can never widen past the instance. Local/CLI/discovered models are free and always selectable (added via `pickableModels`, waved through by the stream's `local === true`). Per-teacher = `instance_settings` (no migration); per-cohort = the `cohorts.allowedModelsJson` column.
